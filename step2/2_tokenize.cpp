...
template <typename it_t>
constexpr auto tokenize(it_t begin, it_t end)
{
  //constexpr auto tokens_count = algo::count(begin, end, ' ');
...
int main()
{
  constexpr auto begin = asm_code.begin();
  constexpr auto end = asm_code.end();

  //constexpr auto tokens = tokenize(begin, end);
  return 0;
}
